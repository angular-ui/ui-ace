...
angular.module('ui.ace', []).value('uiAceConfig', {}).directive('uiAce', ['uiAceConfig', function (uiAceConfig) {
	...
	return {
		...
		link: function (scope, elm, attrs, ngModel) {
				...
				return function (e) {
					...
					if (newValue !== scope.$eval(attrs.value) && !scope.$$phase) {
						...
						if (angular.isDefined(callback)) {
							scope.$apply(function () {
								if (angular.isFunction(callback)) callback(e, acee);
								else throw new Error('ui-ace use a function as callback.');
							});
						}
					}
				};
			...
			// Boolean options
			if (angular.isDefined(opts.showGutter)) acee.renderer.setShowGutter(opts.showGutter);
